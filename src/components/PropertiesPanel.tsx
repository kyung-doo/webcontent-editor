...
// =============================================================================
// StyleRow Component (PropertiesPanel 전용이므로 여기에 둠)
// =============================================================================
...
export default function PropertiesPanel() {
  ...
  const { selectedElementId, canvasSettings } = useSelector((state: RootState) => state.canvas);
  ...
  const selectedElement = elements.find(el => el.elementId === selectedElementId);
  ...
  const refreshAll = useCallback(async (forceReload = false) => {
    ...
    if (selectedElement.scripts) {
        selectedElement.scripts.forEach(scriptPath => {
            ...
            else dispatch(removeScriptFromElement({ id: selectedElement.elementId, scriptName: scriptPath }));
        });
    }
    const schemas: any = {};
  ...
  }, [selectedElement?.elementId, JSON.stringify(selectedElement?.scripts), dispatch]);
  ...
  const commitAttribute = (name: string, value: string) => {
    if (selectedElementId && selectedElement) {
        if ((name === 'id' && value !== selectedElement.id) || 
            ...
            dispatch(updateElementAttributes({ id: selectedElementId, name, value }));
        }
    }
  };
  ...
  const handleCommitStyle = (oldKey: string, newKey: string, newValue: string) => {
    ...
    if (oldKey && oldKey !== newKey) {
      dispatch(updateElementProps({ id: selectedElementId, props: { [oldKey]: undefined, [newKey]: newValue } }));
    } else {
      dispatch(updateElementProps({ id: selectedElementId, props: { [newKey]: newValue } }));
    }
  };
  
  const handleDeleteStyle = (key: string) => {
    if (selectedElementId) dispatch(updateElementProps({ id: selectedElementId, props: { [key]: undefined } }));
  };


  const internalProps = ['className', 'src', 'text', 'scripts', 'scriptValues', 'id', 'elementId', 'children', 'parentId'];
          ...
          <div className="space-y-6 p-5">
            ...
            <div className="space-y-3 border-b pb-4 border-gray-100">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input 
                        ...
                        className="flex-1 min-w-0 border-b border-transparent hover:border-gray-300 focus:border-blue-500 focus:outline-none text-xs text-gray-700 font-mono" placeholder="id"
                    />
                </div>
                <div className="flex items-start gap-2 pt-1">
                    <span className="text-[10px] text-gray-500 font-bold w-8 mt-1.5">.</span>
                    <div className="flex-1 min-w-0">
                      <AutocompleteInput 
                          value={localClass}
                          onChange={setLocalClass}
                          onBlur={() => commitAttribute('className', localClass)}
                          onEnter={() => { commitAttribute('className', localClass); (document.activeElement as HTMLElement)?.blur(); }}
                          options={[]} 
                ...
                <div className="pl-2 border-l-2 border-gray-100">
                    <div className="max-h-80 overflow-y-auto scrollbar-thin pr-1 mb-2">
                        {Object.entries(selectedElement.props).map(([key, value]) => {
                            ...
                            return <StyleRow key={key} propKey={key} propValue={value as string} onCommit={handleCommitStyle} onDelete={handleDeleteStyle} />;
                        })}
                    </div>
                    <div className="border-t border-gray-100 border-dashed pt-1">
                        <StyleRow key="new-row" propKey="" propValue="" onCommit={handleCommitStyle} onDelete={() => {}} isNew={true} />
                    </div>
                </div>
            ...
            {selectedElement.type === 'Text' && (
              <div className="space-y-2 border-t border-gray-100 pt-4">
                  ...
                  <textarea rows={2} value={selectedElement.props.text || ''} className="w-full rounded border border-gray-300 px-2 py-1 text-xs focus:border-blue-500 focus:outline-none resize-none font-mono" onChange={(e) => dispatch(updateElementProps({ id: selectedElementId!, props: { text: e.target.value } }))}/>
              </div>
            )}

            <ComponentInspector selectedElement={selectedElement} availableScripts={availableScripts} scriptSchemas={scriptSchemas} />
          </div>
        )}
    </div>
  );
