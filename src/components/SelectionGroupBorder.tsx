...
export default function SelectionGroupBorder({ selectedIds, paperRef, zoom }: SelectionGroupBorderProps) {
    ...
    useLayoutEffect(() => {
        ...
        selectedIds.forEach((id: string) => {
            ...
            if (node) {
                ...
                descendants.forEach((child: any) => expandByRect(child.getBoundingClientRect()));
            }
        });

        if (hasValid && minX !== Infinity) {
            const P = 4;
            setRect({ x: minX - P, y: minY - P, w: (maxX - minX) + P*2, h: (maxY - minY) + P*2 });
        } else {
            setRect(null);
        }
